<template>
    <WMListSubHeader :filterLabels="filterLabels" :defaultOption="filterLabels[1]" entity="contact" @new="displayNewForm">
    </WMListSubHeader>
    <Sidebar v-model:visible="isDetailsVisible" class="details-sidebar w-6" :showCloseIcon="false" :class="layoutConfig.isRTL.value ? 'layout-rtl' : ''">
        ...
        <Divider />
        <WMCustomersTable :contact="contactDetail" :columns="customerColumns" :showControls="false" :rows="5" />
    </Sidebar>
    <WMNewEntitySidebar name="newContact" entity="contact"  />
    <div class="wm-table-container mt-5 mx-8 flex-auto overflow-auto">
        <DataTable lazy v-model:selection="selectedContacts" :value="contacts" dataKey="contact_id"
                   tableStyle="min-width: 50rem" class="p-datatable-sm" paginator scrollable scrollHeight="flex" :rows="rows"
            ...
            <Column field="contact" header="איש קשר" class="link-col">
                <template #body="slotProps">
                    <router-link :to="{ name: 'contactDetail', params: { 'id': slotProps.data.contact_id } }"
                                 class="vertical-align-middle">{{ slotProps.data.name }}</router-link>
                </template>
            </Column>
...
const loadLazyData = () => {
    ...
    ContactsService.getContactsFromApi({ page: lazyParams.value.page + 1, per_page: listUtilsStore.rows }).then((result) => {
        console.log(result);
