<template>
    ...
    <div v-if="showControls" class="flex flex-column gap-3 mb-3">
        <div class="flex flex-row justify-content-between">
            <div class="flex flex-row">
                <WMButton @click="formUtilsStore.expandSidebar = 'newCustomer'" class="m-1 col-6" name="new" icon="new"
    ...
    <DataTable lazy v-model:filters="filters" v-model:selection="selectedCustomers" :value="customers" dataKey="id"
        ...
        <Column v-for="column in columns" :key="column.name" :field="column.name"
            ...
            <template v-if="column.type === 'star'" #body="slotProps">
                <img v-if="slotProps.data['main_contact']['id'] == contact.id" src="/icons/star.svg" alt="" class="vertical-align-middle">
            </template>
            <template v-if="column.type === 'tags'" #body="slotProps">
                <div class="flex flex-row gap-2">
                    <Tag v-for="item in slotProps.data[column.name]" class="vertical-align-middle">{{ item }}</Tag>
                </div>
            </template>
...
const loadLazyData = () => {
    CustomerService.getCustomersFromApi({ page: lazyParams.value.page + 1, per_page: props.rows, contact_id: props.contact.id })
        .then((result) => {
            console.log(result)
